    ...
    <b-tabs pills card vertical>
      <div v-for="domain in info.items" v-bind:key="domain.domain">
        <b-tab :title="domain.domain">
            <b-card v-if="validLogo(domain.logo)" :title="domain.domain" bg-variant="info" text-variant="white" body-class="max-width:500px" footer="Servers" id="img">
              <b-row no-gutters>
                <b-col cols=3>
            ...
            <b-card v-else :title="domain.domain" bg-variant="info" text-variant="white" body-class="max-width:500px" footer="Servers">
              <b-card-text>
                <b>SSL Grade: </b> {{domain.ssl_grade}} <br>
                <b>Title: </b> {{domain.title}} <br>
                <b>Time: </b> {{domain.time_consulted}} <br>
